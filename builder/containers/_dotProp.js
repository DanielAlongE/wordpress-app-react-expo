...
function set(obj, prop, value) {
	...
	var setPropImmutableRec = function(obj, prop, value, i) {
		...
		var def = {}
		var nextProp = prop[i+1];

		
		//console.log(typeof prop[i+1]);
			
		if(/^\d+$/.test(nextProp)){

			//just to be sure parseInt
			prop[i+1] = Number.parseInt(nextProp);

			//set next default to array
			def = []///\d+/.test(prop[i+1]) ? [] :{};
			//console.log("next prop default is: ", def);
		...
		if (prop.length > i) {
			...
			//console.log("dot", head, i)

			clone[head] = setPropImmutableRec(obj[head] !== undefined ? obj[head] : def, prop, value, i + 1);
			return clone;
		}
